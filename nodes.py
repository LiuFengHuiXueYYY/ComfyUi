...
config_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'model_path.ini')

myconfig = configparser.ConfigParser()
myconfig.read(config_file_path, encoding="utf-8")

print("Available sections:", myconfig.sections())
model_path = myconfig.get('model_path', 'paths')
...
class SaveImageWithMetadata:
    ...
    def INPUT_TYPES(cls):
        return {"required":
                    {"images": ("IMAGE", ),
                     "filename_prefix": ("STRING", {"default": "ComfyUI"})},
                "optional": {"prompt": ("PROMPT", {"default": None}),
                             "extra_pnginfo": ("EXTRA_PNGINFO", {"default": None})}
                }
